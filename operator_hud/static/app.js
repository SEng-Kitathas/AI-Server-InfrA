const state={tools:[],filtered:[],category:'all',selected:null,status:null,pendingApproval:null,lastDispatch:null,hudToken:null};
...
function renderStatus(data){
  state.status=data;
  const h=data.hud||{},r=data.receiver||{},b=data.browser_bridge||{},j=data.journal||{};
  ...
  const good=h.ok&&r.ok&&b.ok&&j.ok; const degraded=h.ok&&(r.ok||b.ok||j.ok);
  const g=$('globalState');g.className=`state-pill ${good?'good':degraded?'degraded':'bad'}`;g.textContent=good?'ALL SYSTEMS NOMINAL':degraded?'DEGRADED':'CONTROL PLANE DOWN';
...
function badgeMarkup(t){return `<span class="badge ${t.mutating?'mutate':'read'}">${t.mutating?'mutating':'read'}</span><span class="badge ${toolNeedsApproval(t)?'approve':'read'}">${toolNeedsApproval(t)?'approval':'no approval'}</span><span class="badge ${String(t.danger_tier).toLowerCase()==='critical'?'critical':'read'}">${escapeHtml(t.danger_tier||'unknown')}</span>`;}
...
function selectTool(t,preset=null){state.selected=t;$('toolEmpty').classList.add('hidden');$('toolDetail').classList.remove('hidden');$('toolCategory').textContent=t.category||'tool';$('toolName').textContent=t.name;$('toolDescription').textContent=t.description||'';$('toolBadges').innerHTML=badgeMarkup(t);buildForm(t.input_schema||{});if(preset)$('rawPayload').value=JSON.stringify(preset,null,2);$('resultBox').textContent='No dispatch yet.';$('resultBox').className='result';renderToolList();}
...
function wire(){
  ...
  $('toolSearch').oninput=applyToolFilter;$('syncFormBtn').onclick=()=>{try{syncFormToJson();}catch(e){showToast(String(e),false);}};$('dispatchBtn').onclick=rawToolDispatch;
