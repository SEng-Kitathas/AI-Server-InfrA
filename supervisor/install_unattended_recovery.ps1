...
$managedTaskNames=@('PCMMAD_V30_Receiver_SYSTEM','PCMMAD_V30_Ngrok_SYSTEM','PCMMAD_V30_Supervisor_SYSTEM','PCMMAD_V30_Daemon_SYSTEM','PCMMAD_V30_DaemonSupervisor_SYSTEM','PCMMAD_V30_NgrokSupervisor_SYSTEM')
...
if($requiresQuiescence){
  ...
  foreach($port in @(5000,4040,$DaemonPort)){if(Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue){throw ('RECOVERY_INSTALL_REQUIRES_RUNTIME_LISTENERS_STOPPED:'+([string]$port))}}
}
...
$rollbackFiles=@('githome_api.dpapi','ngrok_authtoken.dpapi','machine_config.json','start_receiver_system.ps1','start_ngrok_system.ps1','start_supervisor_system.ps1','start_daemon_system.ps1','start_daemon_supervisor_system.ps1','daemon_task_entry.py')
$rollbackFileState=@{}
...
foreach($spec in @(@('PCMMAD_V30_Receiver_SYSTEM','start_receiver_system.ps1'),@('PCMMAD_V30_Ngrok_SYSTEM','start_ngrok_system.ps1'))){
