...
function Test-RealNgrokBinary([string]$Path){
...
$ng=$null
$installedNgrok=Join-Path $InstallRoot 'ngrok.exe'
if(Test-RealNgrokBinary $installedNgrok){$ng=$installedNgrok}
if([string]::IsNullOrWhiteSpace($ng)){$pkg=Get-AppxPackage ngrok.ngrok -ErrorAction SilentlyContinue|Select-Object -First 1;if($pkg){$candidate=Join-Path $pkg.InstallLocation 'ngrok.exe';if(Test-RealNgrokBinary $candidate){$ng=$candidate}}}
if([string]::IsNullOrWhiteSpace($ng)){$proc=Get-Process ngrok -ErrorAction SilentlyContinue|Select-Object -First 1;if($proc -and (Test-RealNgrokBinary $proc.Path)){$ng=$proc.Path}}
if([string]::IsNullOrWhiteSpace($ng)){$cmd=Get-Command ngrok.exe -ErrorAction SilentlyContinue|Select-Object -First 1;if($cmd -and (Test-RealNgrokBinary $cmd.Source)){$ng=$cmd.Source}}
if([string]::IsNullOrWhiteSpace($ng)){throw 'NGROK_REAL_BINARY_NOT_FOUND'}
$ngDest=Join-Path $InstallRoot 'ngrok.exe';if(-not([IO.Path]::GetFullPath($ng).Equals([IO.Path]::GetFullPath($ngDest),[StringComparison]::OrdinalIgnoreCase))){Copy-Item -LiteralPath $ng -Force -Destination $ngDest -ErrorAction Stop}
$nonsecret=@{};foreach($k in @('PCMMAD_ROOT','PCMMAD_PROJECTS_ROOT','PCMMAD_SYSTEM_ROOT','PCMMAD_SANDBOX_ROOT','PCMMAD_TEMP_ROOT','PCMMAD_MOUNTS_JSON','PCMMAD_PROTOCOL_POLICY_MODE','PCMMAD_BIND_HOST','PCMMAD_BIND_PORT','PCMMAD_LOCAL_PORT','PCMMAD_RECEIVER_INSTANCE','PCMMAD_HUD_HOST','PCMMAD_HUD_PORT','PCMMAD_RECEIVER_BASE','PCMMAD_NGROK_URL','PCMMAD_BROWSER_BRIDGE_URL','PCMMAD_GIT_EXE')){$v=[Environment]::GetEnvironmentVariable($k,'User');if(-not[string]::IsNullOrWhiteSpace($v)){$nonsecret[$k]=$v}}
$gitExe=[string]$nonsecret['PCMMAD_GIT_EXE']
if([string]::IsNullOrWhiteSpace($gitExe)-or -not(Test-Path -LiteralPath $gitExe)){$gitExe=$null;$gitCmd=Get-Command git.exe -ErrorAction SilentlyContinue|Select-Object -First 1;if($gitCmd -and (Test-Path -LiteralPath $gitCmd.Source)){$gitExe=$gitCmd.Source}}
if([string]::IsNullOrWhiteSpace($gitExe)){$gitLocal=Join-Path $env:LOCALAPPDATA 'Programs\Git\cmd\git.exe';if(Test-Path -LiteralPath $gitLocal){$gitExe=$gitLocal}}
if([string]::IsNullOrWhiteSpace($gitExe)-or -not(Test-Path -LiteralPath $gitExe)){throw 'GIT_EXECUTABLE_NOT_FOUND_AT_INSTALL'}
$nonsecret['PCMMAD_GIT_EXE']=[IO.Path]::GetFullPath($gitExe)
...
$args=@((Join-Path $root 'receiver_supervisor.py'),'--daemon','--health-url',$url,'--task-name','PCMMAD_V30_Receiver_SYSTEM','--receipt',(Join-Path $root 'receiver_supervisor_receipt.json'),'--stop-file',(Join-Path $root 'receiver_supervisor.maintenance_hold'),'--lock-file',(Join-Path $root 'receiver_supervisor.lock'),'--failure-state-file',(Join-Path $root 'receiver_supervisor.failure_state.json'),'--expected-router');$child=Start-Process -FilePath $py -ArgumentList $args -WorkingDirectory $root -PassThru;try{while(-not $child.HasExited){$state=(Get-ScheduledTask -TaskName 'PCMMAD_V30_Supervisor_SYSTEM' -ErrorAction SilentlyContinue).State;if($null -eq $state -or $state -ne 'Running'){Stop-Process -Id $child.Id -Force -ErrorAction SilentlyContinue;break};Start-Sleep -Milliseconds 500;$child.Refresh()};if($child.HasExited){exit $child.ExitCode}}finally{if(-not $child.HasExited){Stop-Process -Id $child.Id -Force -ErrorAction SilentlyContinue}}
...
$ngrokSupervisorArgs='"'+(Join-Path $InstallRoot 'ngrok_supervisor.py')+'" --health-url http://127.0.0.1:4040/api/tunnels --expected-public-url "'+([string]$nonsecret['PCMMAD_NGROK_URL'])+'" --expected-upstream http://127.0.0.1:5000 --task-name PCMMAD_V30_Ngrok_SYSTEM --receipt "'+(Join-Path $InstallRoot 'ngrok_supervisor_receipt.json')+'" --stop-file "'+(Join-Path $InstallRoot 'ngrok_supervisor.maintenance_hold')+'" --lock-file "'+(Join-Path $InstallRoot 'ngrok_supervisor.lock')+'" --failure-state-file "'+(Join-Path $InstallRoot 'ngrok_supervisor.failure_state.json')+'"'
