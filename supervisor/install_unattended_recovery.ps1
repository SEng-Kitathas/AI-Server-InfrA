...
$nonsecret['LIVE_RECEIVER_ROOT']=$LiveReceiverRoot;$nonsecret|ConvertTo-Json|Set-Content -Encoding UTF8 (Join-Path $InstallRoot 'machine_config.json')
$receiver=@'
...
foreach($spec in @(@('PCMMAD_V30_Receiver_SYSTEM','start_receiver_system.ps1'),@('PCMMAD_V30_Ngrok_SYSTEM','start_ngrok_system.ps1'),@('PCMMAD_V30_Supervisor_SYSTEM','start_supervisor_system.ps1'))){$action=New-ScheduledTaskAction -Execute 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -Argument ('-NoProfile -ExecutionPolicy Bypass -File "'+(Join-Path $InstallRoot $spec[1])+'"');Register-ScheduledTask -TaskName $spec[0] -Action $action -Trigger (New-ScheduledTaskTrigger -AtStartup) -Principal $principal -Settings $settings -Force|Out-Null}
