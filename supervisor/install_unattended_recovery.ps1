...
if(Test-Path $installedNgrok){$ng=$installedNgrok}
if([string]::IsNullOrWhiteSpace($ng)){$cmd=Get-Command ngrok.exe -ErrorAction SilentlyContinue|Select-Object -First 1;if($cmd){$ng=$cmd.Source}}
if([string]::IsNullOrWhiteSpace($ng)){$known=Join-Path $env:LOCALAPPDATA 'Microsoft\WindowsApps\ngrok.exe';if(Test-Path $known){$ng=$known}}
if([string]::IsNullOrWhiteSpace($ng)){$ng=(Get-Process ngrok -ErrorAction SilentlyContinue|Select-Object -First 1 -ExpandProperty Path)}
if([string]::IsNullOrWhiteSpace($ng)-or -not(Test-Path $ng)){throw 'NGROK_REAL_BINARY_NOT_FOUND'}
$ngDest=Join-Path $InstallRoot 'ngrok.exe';if(-not([IO.Path]::GetFullPath($ng).Equals([IO.Path]::GetFullPath($ngDest),[StringComparison]::OrdinalIgnoreCase))){Copy-Item -Force $ng $ngDest}
