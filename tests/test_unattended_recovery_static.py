...
def test_tunnel_failure_cannot_restart_receiver_from_tunnel_launcher():
 s=(ROOT/'supervisor/install_unattended_recovery.ps1').read_text();t=s[s.index("$tunnel=@'"):s.index("'@;Set-Content",s.index("$tunnel=@'"))];assert 'receiver' not in t.lower() or '127.0.0.1:5000' in t.lower();assert 'Stop-Process' not in t and 'Restart' not in t
def test_ngrok_secret_materialized_only_in_acl_temp_and_deleted():
 s=(ROOT/'supervisor/install_unattended_recovery.ps1').read_text();assert 'ngrok-runtime-' in s;assert "'SYSTEM:(F)'" in s;assert 'finally{Remove-Item -Force $tmp' in s
