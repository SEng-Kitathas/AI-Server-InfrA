...
class RuntimeObservabilityTests(unittest.TestCase):
    ...
    def test_telemetry_is_authenticated_and_contains_red_use_fields(self) -> None:
        ...
        self.assertIn(body["status"], {"available", "degraded"})
        self.assertEqual(body["ok"], not bool(body.get("section_errors")))
        ...
        self.assertIn("storage", body["system"])
        if "process" not in body.get("section_errors", {}):
            self.assertGreater(body["process"]["rss_bytes"], 0)
        if "system" not in body.get("section_errors", {}):
