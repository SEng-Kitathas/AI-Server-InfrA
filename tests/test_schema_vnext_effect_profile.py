...
class ProductionEffectProfileTests(unittest.TestCase):
    ...
    def test_profile_exactly_covers_live_capability_catalog(self):
        r=effect.verify_profile(self.profile,self.cards)
        self.assertTrue(r["current"])
        self.assertEqual(r["capability_count"],len(self.cards))
        self.assertGreaterEqual(r["capability_count"],169)
        self.assertEqual(set(x["name"] for x in self.profile["entries"]),set(self.cards))
        self.assertEqual(lab_tools._PLUGIN_ERRORS,[])
        required={"architecture.registry.inspect","architecture.runtime.status","artifact.commit","migration.inspect","migration.plan","migration.execute","migration.verify","derived_state.audit","derived_state.rebuild"}
