...
class ProductionEffectProfileTests(unittest.TestCase):
    ...
    def test_profile_exactly_covers_live_158_capability_catalog(self):
        r=effect.verify_profile(self.profile,self.cards); self.assertTrue(r["current"]); self.assertEqual(r["capability_count"],158); self.assertEqual(set(x["name"] for x in self.profile["entries"]),set(self.cards)); self.assertEqual(lab_tools._PLUGIN_ERRORS,[])
