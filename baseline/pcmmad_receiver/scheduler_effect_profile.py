...
SIDE_EFFECT_TO_KIND = {
    ...
    # Newer declared labels may exist in the live catalog. Unknown labels
    # deliberately fall through to UNVERIFIED rather than being guessed.
    "mutation": "MUTATION",
    "execution": "EXECUTION",
...
def derive_entry(card: Mapping[str, Any]) -> JsonObject:
    ...
    if not side_effect:
        raise LabToolError(
            "EFFECT_PROFILE_UNCLASSIFIED",
            f"capability has no declared side-effect class: {name}",
            409,
        )
    declared_effect_kind = SIDE_EFFECT_TO_KIND.get(side_effect, "UNVERIFIED")
